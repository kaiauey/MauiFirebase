...
using MauiFirestoreAssignment.Models;

namespace MauiFirestoreAssignment.Services;
...
public class FirestoreService
{
    ...
    private async Task SetupFireStore()
    {
        if (db == null)
        {
            var stream = await FileSystem.OpenAppPackageFileAsync("dx212-assignment-firebase-adminsdk-j5cfn-f2c442467a.json");
            var reader = new StreamReader(stream);
            var contents = reader.ReadToEnd();
            db = new FirestoreDbBuilder
            {
                ProjectId = "dx212-assignment",
